...
def on_message(client, userdata, msg):
    ...
    if msg.payload == "master_switch_ON":
        take_input = True
        blink()
    if msg.payload == "timer_stopped":
        print("killing process Timer Stopped" + str(timer_process_id))
    ...
    if msg.payload.startswith("button") and time_delay < time.time() and take_input:
        ...
        if defuse_button is None:
            timer_process_id = Popen(
                ["python", "/home/pi/workspace/pi_timer_python/mqtt_server.py"], preexec_fn=os.setsid).pid
...
def defuseFailure():
    data = {"say": "Bomb defusal failure! Bomb defusal failure! Lock out for 15 seconds",
...
def defuseSuccess(pinLED):
    ...
    data2 = {"say": "Bomb has been defused! Bomb has been defused! Stop, Stop, Stop, Stop stop stop",
             "parms": "-s 140 -ven-us+f3"}

    data = {"say": "Success! Device has been defused!",
            "parms": "-s 140 -ven-us+f3"}
    req = urllib2.Request('http://' + serverIP + ':5000/say')
    req.add_header('Content-Type', 'application/json')
